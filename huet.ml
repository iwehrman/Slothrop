  ...
  let rec max_exp_h pos = function
    ...
    | T(f,ts) -> 
        ...
        in
          maxs (1::vals)
  in
  let result = max_exp_h None t in
    debug_l TRACE (lazy ("max exp of " ^ (string_of_term t) ^ " = " ^ 
...
let checkpoint ((branch,step,time,max) as ms) prob =
  ...
  in
    debug ("Checkpoint: branch=" ^  (string_of_id branch)  ^ 
...
let rec orient ((branch,step,time,max) as ms) = function
  ...
  | (_eqs,unmarked,marked,const) ->
      let dbg_msg = lazy (
        "[ orient: \n" ^ 
          "  eqs: \n" ^
          (string_of_theory (extract _eqs)) ^ "\n" ^
          "  unmarked: \n" ^
          (string_of_trs 
             (RuleHeap.fold 
                (fun ((l1,r1),i1) rs -> (l1,r1)::rs) unmarked [])) ^ "\n" ^ 
          "  marked: \n" ^
          (string_of_trs marked) ^ "\n" ^ 
          " ]")
      in
      let () = debug_l TRACE dbg_msg in
      let ((s,t),_),eqs = choose_equation _eqs in
      let () = debug ("Chose: " ^ (string_of_identity (s,t))) in
      let rules = 
      ...
      let t' = (* norm rules t in *) 
        ...
        if s' = t' then 
          ...
          let () = debug ("Norm: " ^ (string_of_identity (s',t'))) in
...
let rec complete ((branch,step,time,max) as ms) 
  ...
  let size = problem_size prob in
    begin
      debug ("[ Orient: branch=" ^ (string_of_id branch) ^ 
               ", size=" ^ (string_of_int size) ^ 
               ", time=" ^ (string_of_float time) ^ 
               ", max=" ^ (string_of_float max) ^ 
               ", cost=" ^ (string_of_float cost) ^ 
      ...
      in
      let step' = step + 1 in
      let size' = problem_size ([],unmarked',marked',const') in
      let cost' = problem_cost ([],unmarked',marked',const') 
        (branch',step',time',max') 
      in
        debug ("Oriented: branch=" ^ (string_of_id branch') ^ 
                 ", size=" ^ (string_of_int size') ^ 
                 ", time=" ^ (string_of_float time') ^ 
                 ", max=" ^ (string_of_float max') ^ 
                 ", cost=" ^ (string_of_float cost') ^ 
...
let grind _identities _conj find_all =
  ...
  let rec _do_completion conj =
    ...
    let now_t = (Unix.gettimeofday()) -. start_t in
      debug ("[ Open: branch=" ^ (string_of_id branch) ^
               ", size=" ^ (string_of_int size) ^ 
               ", cost=" ^ (string_of_float cost) ^ 
               ", time=" ^ (string_of_float time) ^
               ", max=" ^ (string_of_float max) ^
        ...
        if conj' = [] && not (!completion_mode) then
          ...
          try 
            ...
            let total_t = (Unix.gettimeofday()) -. start_t in
              ...
              debug ("Time/T-call in " ^ (checker_name()) ^ 
                       " (Avg, Min, Max): " ^ 
                       (string_of_float 
                          ((!total_yes_time) /. 
          ...
          with FAIL  -> 
            let total_t = (Unix.gettimeofday()) -. start_t in
              debug ("Close: after=" ^ 
                       (string_of_float (total_t -. now_t)) ^ 
                       ", pending=" ^ 
                       (string_of_int (ProbHeap.size frontier)) ^ 
                       " ]");
              debug ("Time: " ^ 
                       (string_of_float total_t));
              debug ("Orientations possible: " ^ 
                       (string_of_int !orientations));
              debug ("Calls to " ^ (checker_name()) ^ ": " ^ 
                       (string_of_int !call_count));
              debug ("Y,N,M: " ^ 
                       (string_of_int !yes_count) ^ "," ^ 
                       (string_of_int !no_count) ^ "," ^
  ...
  let initial_ms = initial_id,1,0.,0. in
    ...
    if !timeout > 0 then
      begin
        debug ("Setting timer (" ^ (string_of_int !timeout) ^ ")");
    ...
    debug ("Setting max_exp threshold: " ^ 
